...
func (d *DefaultHandler) Handle(_ context.Context, r slog.Record) error {
	...
	// Timestamp.
	if !r.Time.IsZero() && d.opts.withTimestamp {
		// Write the time.
...
// WithSubSystem returns a copy of the given handler but with the new tag. All
// attributes added with WithAttrs will be kept but all groups added with
// WithGroup are lost.
func (d *DefaultHandler) WithSubSystem(tag string) *DefaultHandler {
